...
// 清空访问数据。说明：数据表没有id自动增长字段
$db = new SQLite3(ROOT_PATH . 'dist\\data\\sqlite\\visit.db');
$db->exec('DELETE FROM event;');
$db->exec('VACUUM;');

// 导出开发数据库表结构。说明：用sql创建表重置有id自动增长字段
$db = new SQLite3(ROOT_PATH . 'src\\data\\sqlite\\pagepan-dev.db');
$sql = '';
