...
class Restler {
    ...
    public function handle()
    {
        ...
        else
        {
            try
            {
                ...
                $this->applyClassMetadata(get_class($this->request_format), $this->request_format, $o);
                ...
                {
                    case 3:
                        $reflection_method = new ReflectionMethod($object, $o->method_name);
                        $reflection_method->setAccessible(true);
                        $result = $reflection_method->invokeArgs($object, $o->arguments);
                        break;
                    case 2:
                    case 1:
                    default:
    ...
    public function sendData($data)
    {
        ...
        if (isset($this->service_class_instance) && method_exists($this->service_class_instance, $post_process)
        )
        {
    ...
    {
        $path = urldecode($this->removeCommonPath($_SERVER['REQUEST_URI'], $_SERVER['SCRIPT_NAME']));
    ...
    {
        if (isset($method_info->metadata[$class_name]) && is_array($method_info->metadata[$class_name])
        )
        {
    ...
    protected function generateMap($class_name, $base_path = '')
    {
        ...
        foreach ($methods as $method)
        {
            ...
            if (preg_match_all(
                            '/@url\s+(GET|POST|PUT|DELETE|HEAD|OPTIONS)[ \t]*\/?(\S*)/s', $doc, $matches, PREG_SET_ORDER)
            )
            {
            ...
            elseif ($method_url[0] != '_')
            {
                ...
                if (preg_match_all('/^(GET|POST|PUT|DELETE|HEAD|OPTIONS)/i', $method_url, $matches)
                )
                {
