...
class Admin extends CRUD {
    ...
    protected function postDelete($request_data)
    {
        ...
        foreach ($ids as $id)
        {
            ...
            {
                // 回收站
                $this->_trash($admin, $request_data);
            }
            else
            {
                $error[] = $id;
...
//                }
    ...
    // 重新生成 uikit 缓存文件
    /*private function _updateUikitCache()
    {
        $success = true;
		
        $uikit_conf = loader_config('uikit');
        $uikit_url = $uikit_conf['url'];
        $uikit_path = CACHE_PATH . 'uikit/';
        ...
        $files = file_get_contents($uikit_url . 'getFiles.php', false, $context);
        $files = json_decode($files, true);

        foreach ($files as $name => $code)
        {
            $filename = pathinfo($name, PATHINFO_BASENAME);
            $dirname = pathinfo(parse_url($name, PHP_URL_PATH), PATHINFO_DIRNAME);
            $newpath = $uikit_path . $dirname . '/';

            if ( FS::rmkdir($newpath) )
            {
                file_put_contents($newpath . $filename, $code);
            }
            else
            {
                $success = false;
