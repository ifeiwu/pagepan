...
;(function(window, document) {
    ...
    var popoPicker = function(el, option) {
        ...
        this.Scroll.prototype = {
            ...
            removeItem: function(valArr) {
                ...
                for (var i = 0; i < data.length; i++) {
                    for (var j = 0; j < valArr.length; j++) {
                        if (data[i] && data[i].value == valArr[j]) {
                            //移除
                            data.splice(i--, 1)
                            removeLen++
                        } else {
            ...
            replaceItems: function(valArr) {
                this.opt.data = valArr;
                this.opt.dataLen = this.opt.data.length
                this.scrollTo(rs.result[this.index].value, 0)
            }
        }
    ...
    var popoVisitTime = function(el, opt) {
        new popoPicker('[data-popo-picker]', {
            container: 'body',
            headTitle: '预约时间',
            wheels: [{
                infinite: false,
    ...
    window.popoVisitTime = popoVisitTime
})(window, document)
