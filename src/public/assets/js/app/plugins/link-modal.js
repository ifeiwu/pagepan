    ...
    require(['modal'], function () {
        ...
        $modaljs.on('click', function (e) {
            ...
            if (/\.(png|apng|jpe?g|gif|svg|webp|bmp|ico)(\?.*)?$/.test(href)) {
                $('<div class="modal-image" style="display:none;"><img src="' + this.href + '" /></div>').appendTo('body').modal(defaults);
            }
            // 打开组件
            else if (/^#/.test(href)) {
                $(this).modal(defaults);
            ...
            // 打开iframe
                ...
                let $iframeModal = $('<div class="modal-iframe" style="display:none"><iframe src="' + href + '"></iframe></div>').appendTo('body').modal(defaults);
